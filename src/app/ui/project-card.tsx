    ...
    return (
        <div className={`relative group overflow-hidden ${className}`}>
            ...
            <div className='absolute flex flex-col px-4 py-2 sm:px-8 sm:py-4 gap-2 sm:gap-4 mx-auto w-full h-full sm:transition-none transition-all opacity-0 translate-y-[20px] duration-300 group-hover:translate-y-0 group-hover:opacity-100 group-hover:delay-200'>
                ...
                <div className="flex flex-wrap gap-1 opacity-0 transition-none sm:transition-all translate-x-[20px] duration-300 group-hover:translate-x-0 group-hover:opacity-100 group-hover:delay-150">
                    {data.languages.map(language => (
                        <span key={language} className="text-xs font-normal bg-[#223A72] px-2 py-px sm:py-1 rounded-full">{language}</span>
                    ))}
                </div>

                <div className="flex mb-2 items-end grow gap-4 mx-auto opacity-0 transition-none sm:transition-all translate-y-[20px] duration-300 group-hover:translate-y-0 group-hover:opacity-100 group-hover:delay-150">
                    {data.youtube !== undefined && (
                        <a className="relative flex group/button" target="_blank" href={data.youtube}>
                            <div className='block w-[120px] h-[32px] bg-[#6A080F] rounded transition-none sm:transition-all duration-150 translate-y-0 translate-x-0 group-hover/button:translate-y-[2px] group-hover/button:translate-x-[3px]' />
                            <div className='absolute flex justify-center items-center bg-[#E40818] rounded w-[120px] h-[32px] rotate-0 transition-none sm:transition-all duration-150 translate-y-0 group-hover/button:translate-y-[-5px] group-hover/button:rotate-[2deg]'>
                    ...
                    {data.github !== undefined && (
                        <a className="relative flex group/button" target="_blank" href={data.github}>
                            <div className='block w-[120px] h-[32px] bg-[#44474D] rounded transition-none sm:transition-all duration-150 translate-y-0 translate-x-0 group-hover/button:translate-y-[2px] group-hover/button:translate-x-[3px]' />
                            <div className='absolute flex justify-center items-center bg-[#333539] rounded w-[120px] h-[32px] rotate-0 transition-none sm:transition-all duration-150 translate-y-0 group-hover/button:translate-y-[-5px] group-hover/button:rotate-[2deg]'>
